...
Write a function named listFoods // Solution code here...that takes in the recipe and returns a new array of the food items without any amount or units. Just the name. For example, '1 cup flour' will return 'flour'.
...
const removeEvenValues = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    // console.log(arr[i]);
    if (arr[i] % 2 === 0) {
      // console.log('slicing', arr[i], 'out at ', i);
      arr.splice(i, 1);
      // Set i back one to account for the array length being changed
      i--;
    }
  }
  // console.log(arr);
...
const extractVowels = (str) => {
  ...
  return [newStr, vowels.sort().join('')];
};
