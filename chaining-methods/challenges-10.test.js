...
const count = (target, input) => {
  const total = input.reduce((acc, arr) => {
    arr.forEach(val => acc.push(val));
    return acc;
  }, [])
    .reduce((acc, val) => {
      if (val === target) return acc + 1;
      else return acc;
    },0);
  return total;
};
...
let findMaleAndFemale = (data) => {
  return data.filter(chr => {
    return chr.gender === 'male' || chr.gender === 'female';
  })
    .map(char => char.name)
    .join(' and ');
};
...
let findShortest = (data) => {
  return data.reduce((shortest, char) => {
    if (parseInt(char.height) < parseInt(shortest.height) || shortest.height < 0) {
      shortest = char;
    }
    return shortest;
  }, {height: -1}).name;
