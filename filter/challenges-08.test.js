...
const filterStringsWithVowels = (arr) => {
  return arr.filter(word => /(a|e|i|o|u)/.test(word))};
...
const getStatName = (arr, minBaseStat) => {
  let statNames = [];
  arr.forEach(stat => {
    if (stat.baseStat > minBaseStat) statNames.push(stat.stat.name);
  });
  return statNames;
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(chr => !chr.hasOwnProperty('children'));
//   return arr.filter(chr => chr.children.length > 0);
};
...
const evenOddNumericValues = (arr) => {
  let nums = arr.filter(val => !(isNaN(val)) && typeof(val) != 'string');
  return nums.map(num => (num % 2 ) ? 'odd':'even');
};
