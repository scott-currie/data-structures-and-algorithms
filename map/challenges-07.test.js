...
const forEachTwoToThe = (arr) => {
  let newArr = [];
  arr.forEach(n => newArr.push(Math.pow(2, n)));
  return newArr;
};
...
const charCode = (arr) => {
  return arr.map((c) => c.charCodeAt(0));
};
...
const extractStats = (arr) => {
  return arr.map(stat => {
    console.log(stat);
    let o = {name: stat.stat.name, total: stat.effort + stat.baseStat};
    console.log(o);
    return o;
  });
};
