...
const unenrollBrook = (arr) => {
  return arr.map(roster => {
    return roster.filter(student => !student.toLowerCase().includes('brook'));
  });
};
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach(day => {
    const todaysEvents = [];
    arr.forEach(event => {
      if (event.includes(day)) todaysEvents.push(event);
    });
    dailyEvents.push(todaysEvents);
  });
  return dailyEvents;
};
