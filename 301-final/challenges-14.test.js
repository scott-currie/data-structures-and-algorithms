...
const toTitleCase = (arr) => {
  return arr.map(str => {
    return str.split('').reduce((newStr, chr, i) => {
      const c = (i === 0) ? chr.toUpperCase() : chr;
      return newStr + c; 
    }, '');
  });
};
...
let biggerThanLuke = (arr) => {
  return arr.filter(char => parseFloat(char.mass) > 77)
            .map(char => char.name)
            .join(' - ');
}
...
const detectTicTacToeWin = (b) => {
  for (let r in b) {
    for (let c in b[0]) {
      console.log(r, c);
      if ((b[r][c] === 'X' && b[r][c] === 'X' && b[r][c] === 'X') || (b[r][c] === 'O' && b[r][c] === 'O' && b[r][c] === 'O')) return true;
    }
