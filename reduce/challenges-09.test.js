...
const reversedString = (arr) => {
  return arr.split('').reduce((str, val, i) => {
    return str + arr[arr.length - 1 - i];
  }, '');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((count, chr) => {
    if (chr.hasOwnProperty('children')) {
      count = count + chr.children.length;
    }
    return count;
  }, 0);
};
...
const extractStat = (statName, arr) => {
  return arr.reduce(stat => {
    if (stat.stat.name === statName || stat !== null) {
      return stat;
    }
    else return null;
  });
...
const extractChildren = (arr) => {
  const aNames = arr.filter(chr => {
    return /(a|A)/.test(chr.name);
  });

  return aNames.reduce((children, chr) => {
    if (chr.hasOwnProperty('children')) {
      chr.children.forEach(child => children.push(child));
    }
    return children;
  }, []);
  // console.log(aNames);
